...
public class GameController {
	...
	public GameController() {
		...
		stalemateChecker = new StalemateChecker(this);
	...
	public void squareClicked(Position clickedPosition) {
		...
		AbstractChessPiece clickedPiece = chessBoard.getPieceAtPosition(clickedPosition);
	...
	private void attemptPiecePlacement(Position clickedPosition) {
		...
		if (endGame.isgameOver())
			chessBoard.setAllowUndoOverride(true);
	}
	...
	private void moveCurrentlyHeldPiece(Position clickedPosition) {
		...
		// Game has started so disable undo button choice.
		chessBoard.getAllowUndoCheckBox().setEnabled(false);
	}

	private void attemptToPickUpPiece(AbstractChessPiece clickedPiece) {
	 ...
	 */
	private void addSpecialCases(List<Position> possibleMoves, AbstractChessPiece clickedPiece) {
	...
	private boolean positionIsChecked(Position position) {
		...
		for (Position knightPosition : knightAttackPositions) {
			AbstractChessPiece possibleKnight = chessBoard.getPieceAtPosition(knightPosition);
		...
		for (int xDisp = -1; xDisp < 2; xDisp++) {
			for (int yDisp = -1; yDisp < 2; yDisp++) {
				...
				for (;; x += xDisp, y += yDisp) {
					...
					AbstractChessPiece threateningPiece = chessBoard.getPieceAtPosition(nextPosition);
	...
	private void performCastling(Position clickedPosition) {
		...
		AbstractChessPiece rookToCastle = chessBoard.getPieceAtPosition(rookPosition);
	...
	private boolean isCheckmate() {
		...
		if (gcState.checkBlockingMoves.size() == 1 && gcState.checkBlockingMoves.get(0).equals(gcState.enPassantPosition)) {
			for (int i = -1; i < 2; i += 2) {
				...
				AbstractChessPiece friendlyPawn = chessBoard.getPieceAtPosition(potentialFriendlyPawnPosition);
	...
	public ChessBoardMoment captureCurrentMoment() {
		Map<Position, AbstractChessPiece> chessPieces = chessBoard.getChessPiecesClone();
	...
	public AbstractChessPiece getPieceCurrentlyHeld() {
